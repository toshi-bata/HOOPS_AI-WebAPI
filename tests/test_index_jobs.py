...
Covers:
  ...
  * core.run_index_add_job batching, progress, per-file failure accounting
...
class TestRunIndexAddJob(unittest.TestCase):
    ...
    def _fake(self, errors_for=()):
        def add_to_index(name, file_ids, model=None):
    ...
    def test_input_is_split_into_batches(self):
        self._fake()
        ctx = _FakeCtx()
        core.run_index_add_job(ctx, "idx", [str(i) for i in range(5)], batch_size=2)
    ...
    def test_added_counts_accumulate_across_batches(self):
        self._fake()
        ctx = _FakeCtx()
        core.run_index_add_job(ctx, "idx", [str(i) for i in range(5)], batch_size=2)
    ...
    def test_embed_time_is_recorded(self):
        self._fake()
        ctx = _FakeCtx()
        core.run_index_add_job(ctx, "idx", ["a"], batch_size=2)
        self.assertIn("embed_seconds", ctx.timings)

    def test_cancellation_stops_between_batches(self):
        self._fake()
        ctx = _FakeCtx(cancel_after=1)
        with self.assertRaises(jobstore.JobCanceled):
            core.run_index_add_job(ctx, "idx", [str(i) for i in range(10)], batch_size=2)
        self.assertEqual(len(self.calls), 1)

    def test_empty_input_finishes_cleanly(self):
        self._fake()
        ctx = _FakeCtx()
        core.run_index_add_job(ctx, "idx", [], batch_size=2)
...
class TestJobSettings(unittest.TestCase):
    ...
    def test_batch_size_default_and_override(self):
        self.assertEqual(core.job_batch_size(), 100)
        os.environ["HOOPS_AI_JOB_BATCH_SIZE"] = "10"
        self.assertEqual(core.job_batch_size(), 10)
...
class TestJobEndpoints(unittest.TestCase):
    ...
    def setUp(self):
        ...
        core.add_to_index = lambda name, file_ids, model=None: {
